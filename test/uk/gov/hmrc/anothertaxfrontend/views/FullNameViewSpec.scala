...
class FullNameViewSpec extends ViewSpecBase {
  ...
  "FullNamePage" when {
    "rendering a view with no errors" must {
      ...
      lazy val form = new FullNameForm().form
      ...
      "render a form that submits data to the correct route" in {
        elementAttributes("form") must contain ("action" -> uk.gov.hmrc.anothertaxfrontend.controllers.routes.FullNameController.submit.url)
      }
    ...
    "rendering a view with errors" must {
      ...
      lazy val target: FullNamePage = inject[FullNamePage]
      lazy val form = new FullNameForm().form.bindFromRequest()
      lazy val result: Html = target(form)
      lazy implicit val document: Document = Jsoup.parse(result.body)
      ...
      "have an related to the first name in the error summary" in {
        elementText(""".govuk-error-summary ul.govuk-error-summary__list li a[href="#first"]""") mustBe "This field is required"
      }

      "have an related to the last name in the error summary" in {
        elementText(""".govuk-error-summary ul.govuk-error-summary__list li a[href="#last"]""") mustBe "This field is required"
      }

      "have an error message against the first name in the form" in {
        elementText("form p#first-error") mustBe "Error: This field is required"
      }

      "have an error message against the last name in the form" in {
        elementText("form p#last-error") mustBe "Error: This field is required"
      }
